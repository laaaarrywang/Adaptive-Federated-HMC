...
# Production sweep for d=1000 with exact gradients on a single GPU.
...
DATA=${PROJECT_ROOT}/simulated_bayesian_logistic_regression/data/synthetic_data.mat
OUT_DIR=${SCRATCH_ROOT}/simulated_bayesian_logistic_regression/sweep/d1000_G
mkdir -p "$OUT_DIR"

ITER_TOTAL=2000000   # match leapfrog count across (K, T) settings
M=20
NOISE=0
NSAMP=1000
GPU_ID=${GPU_ID:-0}
...
# FA-LD always K=1, T=10
for eta in "${ETAS[@]}"; do
    CELLS+=("a|fa_ld|$eta|1|10")
done
...
for eta in "${ETAS[@]}"; do
    for K in $(k_candidates "$eta"); do
        CELLS+=("a|fa_hmc|$eta|$K|10")
        Ka=$((K * 10))
        CELLS+=("a|adaptive|$eta|$Ka|1")
    done
done
